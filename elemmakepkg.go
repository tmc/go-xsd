...
func (me *Attribute) makePkg(bag *PkgBag) {
	...
	if len(me.Ref) > 0 {
		...
		tmp = ustr.PrefixWithSep(impName, ".", idPrefix+"HasAttr"+bag.safeName(me.Ref.String()[(strings.Index(me.Ref.String(), ":")+1):]))
		...
		if typeName = me.Type.String(); (len(typeName) == 0) && (len(me.SimpleTypes) > 0) {
			...
			typeName = bag.resolveQnameRef(typeName, "", &impName)
		}
...
func (me *Element) makePkg(bag *PkgBag) {
	...
	if len(me.Ref) > 0 {
		key = bag.resolveQnameRef(me.Ref.String(), "", &impName)
		for pref, cache := range map[string]map[string]string{"HasElem": bag.elemsCacheOnce, "HasElems": bag.elemsCacheMult} {
		...
		if me.Parent() == bag.Schema {
			...
			key = bag.safeName(bag.Stacks.FullName()) + "" + safeName + "" + bag.safeName(typeName) + "" + bag.safeName(defVal)
		}
		...
		for pref, cache := range map[string]map[string]string{"HasElem": bag.elemsCacheOnce, "HasElems": bag.elemsCacheMult} {
			if tmp = idPrefix + pref + key; !bag.elemsWritten[tmp] {
				...
				cache[key] = tmp
				var td = bag.addType(me, tmp, "", me.Annotation)
				td.addField(me, ustr.Ifs(pref == "HasElems", pluralize(safeName), safeName), ustr.Ifs(pref == "HasElems", "[]"+asterisk+typeName, asterisk+typeName), ustr.Ifs(len(bag.Schema.TargetNamespace) > 0, bag.Schema.TargetNamespace.String()+" ", "")+me.Name.String(), me.Annotation)

				if me.parent == bag.Schema {
					loadedSchemas := make(map[string]bool)
					for _, subEl = range bag.Schema.RootSchema([]string{bag.Schema.loadUri}).globalSubstitutionElems(me, loadedSchemas) {
						//td.addField(subEl, "FOOBAR", subEl.Annotation)
...
func (me *Group) makePkg(bag *PkgBag) {
	...
	if len(me.Ref) > 0 {
		if len(bag.elemGroups[me]) == 0 {
			refName = bag.resolveQnameRef(me.Ref.String(), "", &refImp)
			bag.elemGroups[me] = idPrefix + "HasGroup" + refName
			bag.elemGroupRefImps[me] = refImp
		}
	} else {
		me.Ref.Set(me.Name.String())
		safeName := bag.safeName(me.Name.String())
		tmp := idPrefix + "HasGroup" + safeName
...
func (me *List) makePkg(bag *PkgBag) {
	...
	safeName = bag.safeName(st.Name.String())
	...
	if refName := bag.resolveQnameRef(gr.Ref.String(), "", &refImp); !done[refName] {
		if done[refName] = true; len(refImp) > 0 {
			if !strings.HasPrefix(refName, bag.impName+"."+idPrefix) {
				td.addEmbed(gr, refImp+"."+idPrefix+"HasGroup"+refName[(len(refImp)+1):], anns...)
			}
		} else {
			td.addEmbed(gr, idPrefix+"HasGroup"+refName, anns...)
		}
	}
}
