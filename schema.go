...
func (me *Schema) MakeGoPkgSrcFile() (goOutFilePath string, err error) {
	...
	/*
		pkgName := filepath.Base(me.loadLocalPath)
		ext := filepath.Ext(pkgName)
		pkgName = pkgName[0 : len(pkgName)-len(ext)]
		spew.Dump(goPkgPrefix, filepath.Base(me.loadLocalPath))
		var goOutDirPath = filepath.Join(filepath.Dir(me.loadLocalPath), goPkgPrefix+pkgName+goPkgSuffix)
...
func (me *Schema) onLoad(rootAtts []xml.Attr, loadUri, localPath string) (err error) {
	return fmt.Errorf("onLoad: not implemented")
	/*
		var tmpUrl string
		var sd *Schema
	...
	if localCopy {
		...
		if err == nil {
			if sd, err = loadSchemaFile(localPath, uri); sd != nil {
				log.Println("would sd.loadLocalPath = localPath")
